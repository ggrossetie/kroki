...
The unified `color-scheme` option requests a light or dark rendering across diagram types.
It is available like any other diagram option (query parameter, `Kroki-Diagram-Options-Color-Scheme` header or `diagram_options` JSON field).
...
* `dark`
* `auto`
|`light` keeps the current rendering. +
`dark` produces a dark image (dark background, light strokes and text). +
`auto` produces, when supported, a single SVG that adapts to the viewer via a `@media (prefers-color-scheme: dark)` query; otherwise it falls back to `light`.
|===

NOTE: `color-scheme` only provides defaults. A diagram-specific option set explicitly by the caller (for example D2 `theme`/`dark-theme`, Mermaid `theme` or Graphviz `graph-attribute-*`) always takes precedence, as do colors hard-coded in the diagram source.

Support is implemented per diagram type. The currently supported types are:

[cols="1m,1,3",opts="header"]
|===
|Diagram |Level |Notes

|d2 |native |`dark` defaults the theme to a dark one; `auto` emits an adaptive SVG via D2's `--dark-theme`.
|excalidraw |native |`dark` enables Excalidraw's `exportWithDarkMode`; `auto` degrades to `light`.
|goat |native |GoAT's SVG is adaptive out of the box, so `auto` (and `light`) keep that behavior; `dark` forces light strokes for a dark-mode look.
|graphviz / dot |palette |`dark` synthesizes a dark palette via graph/node/edge color attributes; `auto` injects a `prefers-color-scheme` media query (SVG only).
|mermaid |themed |`dark` maps to Mermaid's `dark` theme; `auto` degrades to `light` (Mermaid has no adaptive output).
|nomnoml |palette |`dark` prepends dark `#fill`/`#stroke` directives; author directives win. `auto` degrades to `light`.
|pikchr |native |`dark` enables Pikchr's `--dark-mode` (color inversion); `auto` degrades to `light`.
|plantuml / c4plantuml |themed |`dark` defaults to the `cyborg` theme; `auto` degrades to `light`.
|svgbob |palette |`dark` synthesizes a dark background and light ink; `auto` degrades to `light`.
|vega / vegalite |palette |`dark` merges a dark background and config (axis/legend/title colors) into the spec; author-provided values win. `auto` degrades to `light`.
|===

The set of color schemes honored by each diagram type is also advertised on the `/health` endpoint under the `color_scheme` key (only diagram types that support more than `light` are listed), so clients can discover dark-mode support at runtime.
...
Kroki bundles the https://www.npmjs.com/package/@mermaid-js/layout-elk[ELK] and https://www.npmjs.com/package/@mermaid-js/layout-tidy-tree[tidy-tree] alternate layout engines, which were moved to external packages in Mermaid v11.
You can select one using the `layout` diagram option (e.g., `layout=elk`) or the `layout` property in the diagram frontmatter config.
